...
class StartPage:
    def __init__(self, root=None):
        ...
        self.resized_image = ImageTk.PhotoImage(self.raw_image.resize((100,50), Image.ANTIALIAS))
        tk.Label(self.frame_image, image=self.resized_image).pack()
        self.frame_image.pack(padx=10, pady=10)

        # frame for menu buttons
        self.frame_buttons = tk.Frame(self.main_frame)
        self.frame_buttons.pack(padx=10, pady=10)
        tk.Button(self.frame_buttons, text="New User Registration",
                 command=lambda:[self.main_frame.pack_forget(), self.user_registration.start_page()]).grid(column=0, row=0, sticky='we')
        tk.Button(self.frame_buttons, text="Clear User",
                 command=lambda:[self.main_frame.pack_forget(), self.delete_user.start_page()]).grid(column=1, row=0, sticky='we')
        tk.Button(self.frame_buttons, text="Search User",
                 command=lambda:[self.main_frame.pack_forget(), self.search_user.start_page()]).grid(column=2, row=0, sticky='we')
        tk.Button(self.frame_buttons, text="Show List of User",
                 command=lambda:[self.main_frame.pack_forget(), self.show_users.start_page()]).grid(column=0, row=1, sticky='we')
        tk.Button(self.frame_buttons, text="New Book Registration",
                 command=lambda:[self.main_frame.pack_forget(), self.book_registration.start_page()]).grid(column=1, row=1, sticky='we')
        tk.Button(self.frame_buttons, text="Show List of Book",
                 command=lambda:[self.main_frame.pack_forget(), self.show_books.start_page()]).grid(column=2, row=1, sticky='we')
        tk.Button(self.frame_buttons, text="Seach Book",
                 command=lambda:[self.main_frame.pack_forget(), self.search_book.start_page()]).grid(column=0, row=2, sticky='we')
        tk.Button(self.frame_buttons, text="Loan Book",
                 command=lambda:[self.main_frame.pack_forget(), self.loan_book.start_page()]).grid(column=1, row=2, sticky='we')
        tk.Button(self.frame_buttons, text="Show List of Loan",
                 command=lambda:[self.main_frame.pack_forget(), self.show_loans.start_page()]).grid(column=2, row=2, sticky='we')
        tk.Button(self.frame_buttons, text="Return Book",
                 command=lambda:[self.main_frame.pack_forget(), self.return_book.start_page()]).grid(column=0, row=3, sticky='we')
        tk.Button(self.frame_buttons, text="Show List of Return",
                 command=lambda:[self.main_frame.pack_forget(), self.show_returns.start_page()]).grid(column=1, row=3, sticky='we')
...
class UserRegistration:
    def __init__(self, master=None, app=None):
        ...
        tk.Button(self.frame_button, text="Submit", 
                  command=lambda:[self.input_user(), self.clear_user_registration()]).pack(side=tk.RIGHT)
        self.registration_label = tk.Label(self.frame_button, text="User Registration Succcessfull", fg='green')
    ...
    def input_user(self):
        statement = f"""CALL input_user('{self.var_first_name.get()}',
        ...
        self.registration_label.after(1000, self.registration_label.destroy)    # retain label for 1 second
...
class BookRegistration:
    def __init__(self, master=None, app=None):
        ...
        tk.Label(self.frame_form, text="Pages : ").grid(column=1, row=2, sticky='w')
        ...
        tk.Button(self.frame_button, text="Submit",
                  command=lambda:[self.input_book(), self.clear_book_registration()]).pack(side=tk.RIGHT)
        self.registration_label = tk.Label(self.frame_button, text="Book Registration Succcessfull", fg='green')
    ...
    def input_book(self):
        statement = f"""CALL input_book('{self.var_book_title.get()}',
    ...
    def start_page(self):
        ...
        for i in range(len(self.table)):
            for j in range(len(self.table[0])):
                box = tk.Entry(self.frame_table)
                box.grid(row=i, column=j+1)
                box.insert('end',self.table[i][j])
...
class Search:
    ...
    def search(self):
        ...
        self.data = mysqlcon.retrieve_table(self.procedure)
        for i in range(len(self.data)):
            for j in range(len(self.data[0])):
                self.box = tk.Entry(self.result_frame)
                self.box.grid(row=i+2, column=j+1)
                self.box.insert('end',self.data[i][j])
...
class BookTransaction:
    def __init__ (self, master=None, app=None, transaction_type=None):    # transaction "loan" or "return"
        ...
        tk.Button(self.frame_button, text="Submit",
                  command=lambda:[self.transaction(), self.clear_transaction()]).pack(side=tk.RIGHT)
        self.registration_label = tk.Label(self.frame_button, fg='green')
    ...
    def transaction(self):
        if self.transaction_type == "loan":
            self.registration_label.config(text="Book Loaned")
            statement = f"CALL loan_book({self.var_user_id.get()},{self.var_book_id.get()})" 
        else:
            self.registration_label.config(text="Book Returned")
...
class DeleteUser:
    def __init__(self, master, app):
        ...
        tk.Button(self.frame_button, text="Delete User", 
                 command=lambda:[self.delete_user(),self.clear_delete_user()]).pack(side=tk.RIGHT, padx=5)
        self.registration_label = tk.Label(self.frame_button, text="User Deleted", fg='red')
